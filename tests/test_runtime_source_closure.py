...
def test_current_v12_closure_covers_execution_research_and_construction() -> None:
...
def test_current_v12_closure_survives_git_archive_and_autocrlf(tmp_path: Path) -> None:
    root = Path(__file__).parents[1]
    manifest = position_risk_source_closure_inventory(root)
    source_paths = {
        str(item["path"])
