"""Allow the exact v10-to-v11 runtime input-scope pre-result repair.

Revision ID: 0077_baseline_input_scope
Revises: 0076_baseline_runtime_repair
"""
...
revision: str = "0077_baseline_input_scope"
down_revision: str | None = "0076_baseline_runtime_repair"
...
def downgrade() -> None:
    v5_rows = op.get_bind().scalar(
        ...
        {"contract": _V5_CONTRACT, "generation": _V5_GENERATION},
    )
    if int(v5_rows or 0) > 0:
        raise RuntimeError(
            "cannot downgrade 0077_baseline_input_scope: append-only v5 "
            "runtime input-scope repair evidence exists"
        )
    op.drop_constraint(
        "ck_transparent_baseline_repair_distinct_target",
        "transparent_baseline_pre_result_repairs",
