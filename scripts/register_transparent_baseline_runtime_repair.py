#!/usr/bin/env python3
"""Register the exact production v9-to-v10 runtime-alignment repair."""
...
from quant_platform.transparent_baseline_repair import (
    register_runtime_alignment_repair,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Append the exact three failed v9 no-performance runtime attempts "
            "before opening governed v10 strategy versions"
        )
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "explicit read-only mount of the immutable v9 release whose executed "
    ...
    parser.add_argument(
        "--source-release-commit", default=RUNTIME_ALIGNMENT_SOURCE_COMMIT
    )
    parser.add_argument(
        "--target-recipe-version",
        default=RUNTIME_ALIGNMENT_TARGET_RECIPE_VERSION,
    )
    args = parser.parse_args()

    settings = Settings.from_env(args.env_file.resolve())
    result = register_runtime_alignment_repair(
