...
from quant_platform.horizon_review import resolve_financial_review_trigger
...
def test_financial_trigger_uses_strict_pit_interval_and_does_not_repeat_daily(
    ...
    assert trigger["announcement_date"] == "2026-08-28"
    assert trigger["trigger_effective_date"] == "2026-08-31"
    assert trigger["source_event_count"] == 2
...
def test_promotion_counts_months_and_distinct_pit_report_periods(
    ...
    assert evidence["financial_report_reviews"] == 1
    assert evidence["financial_report_periods"] == ["2025Q4"]
