#!/usr/bin/env python3
"""Register the exact production v16-short to v17 pre-result repair."""
...
from quant_platform.transparent_baseline_repair import (
    register_topk_industry_capacity_repair,
)


def main() -> None:
    only_backtest_id = next(iter(TOPK_INDUSTRY_CAPACITY_SOURCE_BACKTEST_IDS))
    parser = argparse.ArgumentParser(
        description=(
            "Append the exact failed v16 short no-performance attempt before "
            "opening the governed v17 replacement"
        )
    )
    ...
    parser.add_argument(
        ...
        default=TOPK_INDUSTRY_CAPACITY_SOURCE_COMMIT,
    )
    parser.add_argument(
        "--target-recipe-version",
        default=TOPK_INDUSTRY_CAPACITY_TARGET_RECIPE_VERSION,
    )
    ...
    result = register_topk_industry_capacity_repair(
