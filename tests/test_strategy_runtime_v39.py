...
def test_v39_seals_the_unchanged_runner_and_corrected_audit_consumer() -> None:
    assert RECIPE_VERSION == "qlib-rdagent-single-mainline-2026-09-06-v39"
...
def test_v39_bound_job_checks_current_runner_bundle_and_worker_image(
    ...
    with pytest.raises(ValueError, match="transparent v39 worker runtime image differs"):
        runtime.require_transparent_baseline_runner(
            config=config, job_payload=payload, runner_path=runner_path
        )
...
def test_v39_current_admission_binds_release_without_weakening_sealed_oos(
    monkeypatch: pytest.MonkeyPatch, recipe_id: str,
) -> None:
    image = "sha256:" + "1" * 64
    monkeypatch.setenv(runtime.WORKER_RUNTIME_IMAGE_DIGEST_ENV, image)
    ...
    for mode in ("pre_registered_replay", None):
        with pytest.raises(ValueError, match="v39.*requires sealed final OOS"):
    ...
    with pytest.raises(ValueError, match="v39 worker runtime image digest is missing"):
