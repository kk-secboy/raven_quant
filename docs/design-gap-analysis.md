...
- 本次代码修复已把 `historical_validation_periods` 与最终 backtest
  ...
  分钟回测。线上尚未产生满足该 v4 合同的正式制品，因而“2008 至今已经
  验证通过”仍不成立。
...
| 正式验证 | 代码已修复、线上制品未完成 | v4 合同分离至少 2520 个交易日的预最终历史、一次性最终 OOS 和近期分钟执行；完整候选矩阵继续使用 Holm/PBO/DSR，矩阵不可重建的 factor 家族明确将 PBO/DSR 标为不可计算，只允许真实试验数 Bonferroni、正 bootstrap 下界与完整外层 walk-forward 的保守替代门；仍待 2008 数据集和正式制品验收 |
...
- `formal_validation.py` 提供 Holm 多重检验、PBO、moving-block bootstrap、
  同区间配对比较、外层 walk-forward 聚合、消融和延迟衰减门。历史 factor
  候选矩阵不可重建时不伪造 PBO/DSR：真实试验数与台账哈希绑定，当前冻结策略
  必须通过完整预最终外层 walk-forward、正 bootstrap 置信区间下界和 Bonferroni
  家族校正；完整 model/quant 候选矩阵的原合同不变。
- `run_multifactor_backtest.py` 把正式验证写入冻结 artifact；正式批准读取
  这些结果，缺字段或失败状态均不能晋升。
- v4 正式验证合同把预最终历史与最终锁箱作为两段不可变日期证据：
...
- 完整候选矩阵的多候选路径仍要求每个 Fold 的候选制品，不能把“同一冻结
  策略分段运行”冒充重新训练、验证和选择的完整嵌套 walk-forward。仅当历史
  factor 候选矩阵不可重建时，才允许把当前冻结策略的外层运行明确标成
  `selection_performed=false`，并叠加真实试验数 Bonferroni 与正 bootstrap
  区间；这不是完整嵌套选择证据，PBO/DSR 也继续标为不可计算。
