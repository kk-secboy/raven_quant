...
def _config(
    ...
    if recipe_version == STRATEGY_RESEARCH_V21_TARGET_RECIPE_VERSION:
        bootstrap[TRANSPARENT_BASELINE_RUNTIME_BUNDLE_FIELD] = (
            STRATEGY_RESEARCH_V21_TARGET_RUNTIME_BUNDLE_SHA256
        )
        bootstrap[TRANSPARENT_BASELINE_WORKER_RUNTIME_IMAGE_FIELD] = (
    ...
    if recipe_version == STRATEGY_RESEARCH_V22_TARGET_RECIPE_VERSION:
        bootstrap[TRANSPARENT_BASELINE_RUNTIME_BUNDLE_FIELD] = (
            STRATEGY_RESEARCH_V22_TARGET_RUNTIME_BUNDLE_SHA256
        )
        bootstrap[TRANSPARENT_BASELINE_WORKER_RUNTIME_IMAGE_FIELD] = (
...
def test_historical_v18_identity_is_not_rebound_to_current_v23_runtime() -> None:
    runner = Path(__file__).parents[1] / "scripts" / "run_multifactor_backtest.py"
...
def test_historical_v20_identity_is_not_rebound_to_current_v23_runtime() -> None:
    runner = Path(__file__).parents[1] / "scripts" / "run_multifactor_backtest.py"
...
def test_historical_v21_identity_is_not_rebound_to_current_v23_runtime() -> None:
    runner = Path(__file__).parents[1] / "scripts" / "run_multifactor_backtest.py"

    assert target_runner_for_recipe(
        "short_relative_strength", STRATEGY_RESEARCH_V21_TARGET_RECIPE_VERSION
    ) == STRATEGY_RESEARCH_V21_TARGET_RUNNER_SHA256
    assert target_runtime_bundle_for_recipe(
        "short_relative_strength", STRATEGY_RESEARCH_V21_TARGET_RECIPE_VERSION
    ) == STRATEGY_RESEARCH_V21_TARGET_RUNTIME_BUNDLE_SHA256
    with pytest.raises(ValueError, match="transparent v21 runtime bundle differs"):
        require_transparent_baseline_runner(
            config=_config(
                STRATEGY_RESEARCH_V21_TARGET_RECIPE_VERSION,
                STRATEGY_RESEARCH_V21_TARGET_RUNNER_SHA256,
...
def test_historical_v22_identity_is_not_rebound_to_current_v23_runtime() -> None:
    runner = Path(__file__).parents[1] / "scripts" / "run_multifactor_backtest.py"

    assert target_runner_for_recipe(
        "short_relative_strength", STRATEGY_RESEARCH_V22_TARGET_RECIPE_VERSION
    ) == STRATEGY_RESEARCH_V22_TARGET_RUNNER_SHA256
    assert target_runtime_bundle_for_recipe(
        "short_relative_strength", STRATEGY_RESEARCH_V22_TARGET_RECIPE_VERSION
    ) == STRATEGY_RESEARCH_V22_TARGET_RUNTIME_BUNDLE_SHA256
    with pytest.raises(ValueError, match="transparent v22 runtime bundle differs"):
        require_transparent_baseline_runner(
            config=_config(
                STRATEGY_RESEARCH_V22_TARGET_RECIPE_VERSION,
                STRATEGY_RESEARCH_V22_TARGET_RUNNER_SHA256,
...
def test_current_v23_identity_is_bound_and_executable() -> None:
    runner = Path(__file__).parents[1] / "scripts" / "run_multifactor_backtest.py"
    config = _config(
        STRATEGY_RESEARCH_TARGET_RECIPE_VERSION,
        STRATEGY_RESEARCH_TARGET_RUNNER_SHA256,
...
def test_current_v23_job_identity_cannot_be_changed() -> None:
    config = _config(
        STRATEGY_RESEARCH_TARGET_RECIPE_VERSION,
        STRATEGY_RESEARCH_TARGET_RUNNER_SHA256,
    )
...
def test_v18_runtime_bundle_survives_git_archive_with_autocrlf(tmp_path: Path) -> None:
    ...
    observed = position_risk_bundle_sha256(root)
    assert position_risk_bundle_sha256(repo) == observed
    assert position_risk_bundle_sha256(archive_root) == observed
    assert observed != STRATEGY_RESEARCH_V22_TARGET_RUNTIME_BUNDLE_SHA256
    assert STRATEGY_RESEARCH_TARGET_RUNTIME_BUNDLE_SHA256 in {"0" * 64, observed}
...
def test_repair_migrations_pin_historical_and_current_runner_identities() -> None:
    ...
    v22_migration = (
        ...
        / "migrations"
        / "versions"
        / "0093_strategy_runtime_v22.py"
    ).read_text(encoding="utf-8")
    v23_migration = (
        Path(__file__).parents[1]
        / "migrations"
        / "versions"
        / "0094_strategy_runtime_v23.py"
    ).read_text(encoding="utf-8")
    database_metadata = (
    ...
    assert STRATEGY_RESEARCH_V21_TARGET_RECIPE_VERSION in v21_migration
    assert STRATEGY_RESEARCH_V21_TARGET_RUNNER_SHA256 in v21_migration
    assert STRATEGY_RESEARCH_V21_TARGET_RUNTIME_BUNDLE_SHA256 in v21_migration
    assert "down_revision: str | None = \"0091_strategy_runtime_v20\"" in v21_migration
    assert STRATEGY_RESEARCH_V22_TARGET_RECIPE_VERSION in v22_migration
    assert STRATEGY_RESEARCH_V22_TARGET_RUNNER_SHA256 in v22_migration
    assert STRATEGY_RESEARCH_V22_TARGET_RUNTIME_BUNDLE_SHA256 in v22_migration
    assert "down_revision: str | None = \"0092_strategy_runtime_v21\"" in v22_migration
    assert STRATEGY_RESEARCH_TARGET_RECIPE_VERSION in v23_migration
    assert STRATEGY_RESEARCH_TARGET_RUNNER_SHA256 in v23_migration
    assert STRATEGY_RESEARCH_TARGET_RUNTIME_BUNDLE_SHA256 in v23_migration
    assert "down_revision: str | None = \"0093_strategy_runtime_v22\"" in v23_migration
    assert STRATEGY_RESEARCH_V20_TARGET_RUNTIME_BUNDLE_SHA256 in database_metadata
    assert STRATEGY_RESEARCH_V21_TARGET_RUNTIME_BUNDLE_SHA256 in database_metadata
    assert STRATEGY_RESEARCH_V22_TARGET_RUNTIME_BUNDLE_SHA256 in database_metadata
