...
- 线上既有日线 Qlib 制品主要覆盖 2024—2026；2018—2026 原始全量回补仍在
  ...
  冻结日线研究数据集。
- 研究入口不再内置固定日期。平台按所选不可变 Qlib 快照的真实交易日历，
  让 Qlib 并发复算近 3 年近期型、近 5 年均衡型和近 10 年稳健型证据；
  近期结果主导，均衡结果必须通过，长期结果负责方向一致和不崩溃检查。
  三组共同保留 5 个隔离日和 252 个最终样本外交易日；每次任务或活动创建
  时冻结实际日期和数据集身份，最终样本只统一打开一次，RD-Agent 无权选择
  或改动。三组结果必须形成带哈希的显式共识准入记录，晋级不依赖评估写入
  顺序。持续研究下一轮必须等待完整的新最终样本外区间；同一研究计划任何
  已保留或已消费的 OOS 日期重叠都会被台账拒绝。滚动策略只公开最终 OOS
  长度和 embargo，3/5/10 年验证窗由版本化平台策略统一决定，不再暴露实际
  不生效的训练/验证天数输入。新闻类接口按真实历史起点裁剪，分钟历史不冒充长期覆盖。但这些
  是计划和门槛，尚不是已完成的数据或回测成绩。
