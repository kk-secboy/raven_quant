...
def build_document() -> Document:
    ...
    doc.core_properties.comments = (
        "Version 3.0 final product, strategy, risk and acceptance baseline"
    )
    ...
    add_table(
        ...
        [
            ...
            [
                "adj_factor",
                "复权因子",
                "因子与收益使用 raw_price × adj_factor；不得使用未来公司行动",
            ],
            [
                "财务数据",
                "公告与报告期并存",
                "最早在实际公告日之后可用；修订按当时可见版本进入新快照",
            ],
            ["行业/指数成分", "纳入、剔除日期", "按交易日 PIT 展开，禁止用当前成分回填历史"],
            [
                "停牌/退市",
                "状态与日期",
                "停牌不成交；退市证券保留在历史股票池并按真实可交易阶段处理",
            ],
        ],
        [1900, 3000, 4460],
    )
    ...
    add_table(
        ...
        [
            ...
            [
                "市场冲击",
                "25 bps × sqrt(订单额 / 20日平均成交额)",
                "与基础滑点相加，单边最高 100 bps",
            ],
    ...
    add_list(
        doc,
        [
            "时间切分按 Qlib 交易日历滚动生成：近 3 年、近 5 年、近 10 年并发验证，最终测试至少 252 个交易日；所有边界在研究开始前冻结，调用方只配置最终测试长度和 embargo，不得提交名存实亡的训练/验证天数。",
            "每个研究活动最多 100 个候选试验；单个候选最多 30 组参数。所有成功、失败和被丢弃试验均计入试验次数，不得只登记赢家。",
            "最终测试集对同一 experiment_family 只允许解封一次。三组验证必须形成绑定代码、因子值、数据身份、共同 OOS 和三份评估证据的显式共识记录后才能晋级；查看最终测试结果后再调参，必须建立新研究活动并使用更晚且完全不重叠的测试区间。",
    ...
    add_table(
        ...
        [
            [
                "action",
                "BUY / SELL / HOLD / EXIT / NO_ACTION",
                "明确动作；无合格机会时必须允许 NO_ACTION",
            ],
    ...
    add_list(
        doc,
        [
            "两个核心策略分别至少 252 个最终样本外交易日，并保留预最终十年滚动验证证据。",
    ...
    add_table(
        ...
        [
            [
                "第一阶段",
                "Tushare日线/PIT数据、Qlib数据集、指数增强、成本与正式回测",
                "指数增强通过全部准入线",
            ],
            [
                "第二阶段",
                "全市场行业中性多因子、组合风险与正式推荐",
                "两个核心均通过并可生成可复算推荐",
            ],
            [
                "第三阶段",
                "500万元统一模拟账户、订单成交、NAV、回撤、告警、恢复",
                "连续60日模拟和120日回放通过",
            ],
