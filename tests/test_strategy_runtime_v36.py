...
def test_v36_migration_and_metadata_preserve_the_previous_runtime_seals() -> None:
    ...
    for version, migration in zip((33, 34, 35), migrations[:-1], strict=True):
...
def test_v36_seal_normalization_keeps_economic_source_changes_visible() -> None:
    ...
    changed_seal = source.replace(
        runtime.STRATEGY_RESEARCH_V35_TARGET_RUNTIME_BUNDLE_SHA256, "0" * 64,
    )
    ...
    for relative in ("src/quant_platform/eligibility.py", "scripts/run_multifactor_backtest.py"):
        source = (ROOT / relative).read_text(encoding="utf-8")
