...
HEAD = "0113_strategy_runtime_v41"
...
def runtime_probe(database_url: str) -> Iterator[Connection]:
    ...
    try:
        with engine.begin() as connection:
            ...
            assert {V40_CHECK, V41_CHECK} <= definitions.keys()
            connection.execute(text(
                "CREATE TEMPORARY TABLE v41_runtime_check_probe ON COMMIT DROP AS "
                "SELECT config_json, evidence_mode FROM quantlab.strategy_versions WITH NO DATA"
            ))
            for name in (V40_CHECK, V41_CHECK):
                # Both names are constants; each expression comes from this test DB's catalog.
                connection.execute(text(
                    f"ALTER TABLE pg_temp.v41_runtime_check_probe "
...
def _config(recipe_id: str, *, historical: bool = False) -> dict[str, Any]:
    prefix = "STRATEGY_RESEARCH_V40_TARGET_" if historical else "STRATEGY_RESEARCH_TARGET_"
    return {
        "recipe_id": recipe_id,
...
def _insert(connection: Connection, config: dict[str, Any], mode: str | None) -> None:
    connection.execute(text(
        "INSERT INTO pg_temp.v41_runtime_check_probe (config_json, evidence_mode) "
...
def test_migrated_checks_admit_all_current_and_historical_recipes(
    ...
    assert runtime_probe.execute(text(
        "SELECT count(*) FROM pg_temp.v41_runtime_check_probe"
    )).scalar_one() == 6
    assert runtime_probe.execute(text(
        "SELECT count(*) FROM pg_temp.v41_runtime_check_probe "
        ...
        "version": runtime.STRATEGY_RESEARCH_V40_TARGET_RECIPE_VERSION,
        "bundle": runtime.STRATEGY_RESEARCH_V40_TARGET_RUNTIME_BUNDLE_SHA256,
    }).scalar_one() == 3


def test_migrated_v41_check_rejects_incomplete_or_mismatched_identity(
    runtime_probe: Connection,
) -> None:
    for recipe_id in RECIPES:
        ...
        cases.append(("old v40 bundle", old_bundle, "sealed_final_oos"))
        ...
        cases.append(("unaccepted v41 candidate", unaccepted_candidate, "sealed_final_oos"))
        ...
        for label, config, mode in cases:
            ...
            assert rejected.value.orig.diag.constraint_name == V41_CHECK, (recipe_id, label)
    assert runtime_probe.execute(text(
        "SELECT count(*) FROM pg_temp.v41_runtime_check_probe"
    )).scalar_one() == 0


def test_v41_real_migration_roundtrip_preserves_all_historical_checks(
    database_url: str,
) -> None:
    engine = open_database(database_url)
    ...
    try:
        with engine.connect() as connection:
            ...
            assert {V40_CHECK, V41_CHECK} <= before.keys()
        command.downgrade(config, "0112_strategy_runtime_v40")
        with engine.connect() as connection:
            assert _head(connection) == "0112_strategy_runtime_v40"
            after_downgrade = _checks(connection)
            assert after_downgrade == {
                name: definition for name, definition in before.items() if name != V41_CHECK
            }
            assert after_downgrade[V40_CHECK].encode() == before[V40_CHECK].encode()
