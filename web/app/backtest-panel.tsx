  ...
  return <>
    ...
    {view === "create" && <section className="backtest-hero">
      ...
      <form className="backtest-launcher" onSubmit={runBacktest}>
        ...
        <div className="execution-note"><b>不可变执行契约</b><span>信号与执行频率写入策略版本，固定滞后一 Bar，禁止同 Bar 成交</span><span>15/30/60 分钟只由 Qlib 从分钟数据重采样，不增加下载线路</span><span>审批至少需要 252 个最终样本外交易日；外层滚动验证使用预最终十年历史</span><span>额外运行双倍成本、收紧换手/持仓和容量压力测试</span><span>回测、回放和模拟盘必须共享同一执行契约哈希</span></div>
