...
- **P3**：财务修订先按每行 `available_at=max(ann_date, f_ann_date/actual_date)` 保留原始披露与后续修订，再对每个交易日只选择当时已可见的最新报告期及其最新修订；禁止用未来 `f_ann_date` 全局去重后回填到旧 `ann_date`。缺少可信修订披露日的冲突只能以行级 `ingested_at` 作为保守可见上界，且明确标记 `available_at_source=ingested_at_upper_bound`；连可靠采集时间也没有的冲突直接阻断构建。测试：`tests/test_qlib_builder.py::test_fundamental_revision_becomes_visible_only_after_f_ann_date`、`test_undated_revision_uses_ingested_at_only_as_future_upper_bound`、`test_appending_undated_revision_cannot_change_prior_trade_dates`、`test_unsequenced_financial_revision_conflict_fails_closed_across_row_order`。
