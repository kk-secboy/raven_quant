...
当前受管策略运行时为 `qlib-rdagent-single-mainline-2026-09-05-v34`：风格数据门仅适用于
策略实际消费的风格输入；模拟订单在统一账户时钟上执行并共享同证券同分钟容量。
本次发布新增数据库迁移 `0105_strategy_runtime_v34`，绑定新的 runner、完整源码闭包和
精确 worker 镜像身份。v33 及更早身份和证据保持只读，不得改写旧版本或拼接模拟阶段。
