...
QUANT_BUNDLE_CONTRACT_VERSION = "quant-bundle-ablation-v1"
...
def build_run_multiple_testing_evidence(
    *,
    ...
    if len(definitions) == 1:
        ...
        pbo = probability_of_backtest_overfitting(trials, blocks=8)
        pbo_passed = (
            pbo.get("status") == "ok"
    ...
    sharpes = [
        float(trials[name].mean() / trials[name].std(ddof=1))
        for name in trials.columns
    ]
...
def validate_quant_bundle_evidence(
    ...
    *,
    ...
    if bundle.get("contract_version") != QUANT_BUNDLE_CONTRACT_VERSION:
