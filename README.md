...
当前受管策略运行时为 `qlib-rdagent-single-mainline-2026-09-06-v37`：整手取整必须留在
原持仓的容量与买卖方向可行区间内，不能因卖出目标向下取整而多卖一手；无可行整手目标
仍失败关闭，离散后的全部硬约束继续复检。该执行修复会改变触发场景的交易结果，必须
重新计算受影响回测，不能沿用旧结果宣称等价。v36 的 PIT 证券资格和 Qlib 截面查找
准备机制继续复用固定输入。可转债评级 `cb_rating` 继续按日刷新请求桶，同一桶复用检查点，
新桶追加请求，保留原查询参数和历史快照。
风格数据门仍仅适用于策略实际消费的输入；模拟订单仍按统一账户时钟共享同证券同分钟容量。
本次发布新增数据库迁移 `0108_strategy_runtime_v37`，绑定 runner、完整源码闭包和
精确 worker 镜像身份。v36 及更早身份和证据保持只读，不得改写旧版本或拼接模拟阶段，
