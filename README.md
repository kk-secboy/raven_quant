...
研究经验检索使用已配置的 Embedding 凭据。智谱 `embedding-3` 的长代码和反馈文本按
UTF-8 边界分段请求，并按内容长度聚合为一个检索向量；知识节点保留完整原文，不通过
截断经验、伪造向量或关闭 CoSTEER 来绕过接口输入限制。其他 Embedding 提供方沿用原调用。
...
隔离模拟盘；满足各周期真实前向门后由系统原子自动晋级，人工只能紧急暂停或回滚。研究与回测阶段的统计指标（bootstrap、PBO、DSR、显著性校正）照常计算并密封入档但不参与否决（宽进严出）；模型锦标赛、外部 NLP 因子评估和因子 SOTA 增量适用同一口径，统计显著性一律只入档；前向晋级由真实模拟盘表现决定，进入隔离模拟盘前仍须满足正式 OOS 衰减、数据完整性和执行成本约束。

完整主线活动通过同一个研究入口显式冻结 `completion_mode=managed_fin_strategy`：模型选拔、RD-Agent 多轮研究与独立验证完成后，由现有 scheduler 把同一数据身份和冠军信号交给受管 `fin_strategy`，继续 `policy_only → full_stack → 正式 OOS → 隔离模拟盘`。默认 `research_only` 和旧活动不会自动扩展权限。前向验证从账户实际建立后累计真实交易日，启动模拟盘不代表前向验证通过；正式 OOS 的衰减要求、真实执行约束、数据完整性和成本合同仍需满足。
...
当前受管策略运行时为 `qlib-rdagent-single-mainline-2026-09-14-v44`。新增研究活动入口
`POST /api/autopilot/research-events`，通过幂等活动键在原有主线上注册新的有界研究，
服务端冻结真实数据、完整配置和预算，由现有 scheduler 推进模型选拔、`fin_quant` 与
独立验证。新活动不会重开或覆盖旧终态；全局暂停和所有准入条件继续生效。
迁移 `0111_autopilot_research_events` 增加活动身份，历史 `0112_strategy_runtime_v40`
保持原样；`0113_strategy_runtime_v41` 封存分块模型数据处理、完整活动衔接与效果报告规则。
`0114_strategy_runtime_v42` 独立封存可信预处理缓存，复用同一冻结数据、特征与训练窗口的
预处理制品，各模型与种子仍独立训练、预测和回测；策略规则与准入条件不变。
同一版本还封存实验单元调度、显式计算线程和完整单元回执：并发在原有整批资源预算内
按固定档位分配，成功单元经输入与输出全量校验后可在同一任务的后续 attempt 复用，
已失败单元保留失败，不增加统计重试机会。持续研究维持短线周、中线月、长线季度节奏，
完整模型重选分别按月、季度、年进行；无兼容冠军、显式完整活动或契约迁移仍完整选拔。
其余研究活动沿用已封存冠军配方，并在当前数据上完成独立重验后继续多轮研究。
`0115_strategy_runtime_v43` 封存模型计算的进度观察合同：固定总耗时不再触发自动终止，
长时间或未观察到进展只提示检查；用户取消、内存硬限额和真实异常仍停止执行。
重复资源预检并入首个正式实验格，成功制品直接计入原有三窗口、三种子实验集合，
不增加额外训练、统计机会或 OOS 权限。RD-Agent 主动研究预算与网络超时保持各自含义。
`0116_strategy_runtime_v44` 单独封存因子实现历史与研究内存预算修复；v43 的数据库约束、
源码身份和历史结果保持不变。因子输入覆盖原始提交历史且不缩短冻结训练区间，正式样本外
仍不可见。已经结算的重算失败通过新验证活动重验原封存代码和数值，不复用旧统计结论。
`fin_quant` 的主动研究时长在新一轮开始前检查；已开始的实验完成训练、反馈和记录后交回结果，
外层不再用“研究时长加五分钟”强制终止。因子代码隔离限额和结果导出超时继续保留。
基线特征在固定 Qlib Docker 镜像中校验，校验失败即报错，不能静默改用默认特征。
因子快照中的旧输出先移到隔离执行目录之外保存，每次执行必须重新生成当前输出。
执行和运维边界见 [统一研究提速](docs/model-execution-performance.md)。
维护取消后的受管补充数据下载可沿原任务续跑，保留断点、进度和已消耗尝试；
仅限明确的数据命令白名单，恢复与审计同事务完成，不适用于模型或策略任务。
v42 及更早的身份、结果、迁移和 OOS 消费记录保持原样，冻结活动不能由新版本热接管。

v39 修正参数实验创建端
与阶段结算端的审计字段一致性：两端按同一冻结计划构造完整 periods 与 competition 摘要，
仍严格校验治理字段、数据身份和阶段身份。发布新增 `0110_strategy_runtime_v39`，runner
字节保持不变，源码闭包单独封存。v38 及更早任务、结果和 OOS 消费记录保持只读；通过
正常新任务验证修复，不改写旧任务状态，不把旧成功段接入新任务，也不新增 OOS 权限。

v38 已修正 Qlib 持仓数量与执行边界：Qlib 持仓的内部数量
...
撮合取整继续受每次请求、成交量和现金裁剪后的上限约束，不用固定股数补偿扩大成交。
...
信号衰减的有限非正基准保留为经济检验未通过，完整运行各延迟场景，且没有可准入的
有效延迟；不再把这种结果误作程序执行异常，缺失或非有限指标仍报错。
参数实验的预终审结果不生成正式策略健康参考，也不登记不存在的健康参考制品。
正式回测与既有历史回放仍须绑定真实回测编号和规则摘要；不能用默认身份补齐。
参数回测的运行日志置于段制品目录之外，避免日志追加使已封存的制品校验失效。
v36 的 PIT 证券资格和 Qlib 截面查找
准备机制继续复用固定输入。可转债评级 `cb_rating` 继续按日刷新请求桶，同一桶复用检查点，
新桶追加请求，保留原查询参数和历史快照。
风格数据门仍仅适用于策略实际消费的输入；模拟订单仍按统一账户时钟共享同证券同分钟容量。
v38 的数据库迁移 `0109_strategy_runtime_v38` 原样保留，绑定当时的 runner、完整源码闭包和
