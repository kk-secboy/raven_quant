"""Sealed transparent-baseline evidence contracts (joint lockbox and receipts).

These symbols were lifted verbatim out of the retired
``transparent_baseline_lockbox`` module during weight-reduction phase C3.  The
standalone transparent-baseline line (bootstrap scheduler, repair registrars,
interruption recovery runner, ``TransparentBaselineLockboxStore``) has been
deleted; the public baselines survive only as control groups inside the
managed ``fin_strategy`` competition.  This module carries the remaining
*live* governance surface:

- ``replay_governance`` imports ``BOOTSTRAP_CONFIG_KEY``,
