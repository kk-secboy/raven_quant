...
class AllocationStore:
    """Governed low-correlation strategy allocations backed by paper ledgers."""
    ...
    def _daily_returns(backtest: dict[str, Any]) -> pd.Series:
        ...
        if date_column is None or "net_return" not in frame.columns:
        ...
        with self.engine.connect() as connection:
            for version_id in version_ids:
                ...
                if version["status"] != "approved":
                    raise ValueError("strategy allocations require approved strategy versions")
                ...
                if backtest_row is None:
                    raise ValueError(
                        f"strategy version {version_id} has no successful {dataset} backtest"
                    )
                ...
                return_series[version_id] = self._daily_returns(backtest)
                members.append(
                    {
        ...
        try:
            with self.engine.begin() as connection:
                ...
                for member in rows:
                    ...
                    initial_cash = _decimal(allocation.total_capital) * _decimal(
                        member.target_weight
                    )
                    if initial_cash < Decimal("100000"):
                        raise ValueError(
                            f"allocated capital for {strategy_name} is below the 100000 minimum"
                        )
                    portfolio_id = uuid.uuid4().hex
                    portfolio_name = (f"{allocation.name} · {strategy_name} v{version['version']}")[
                        :150
                    ]
                    connection.execute(
            ...
            raise ValueError("strategy allocation child portfolio name conflicted") from exc
        return self.get(allocation_id)
    ...
    def set_status(self, allocation_id: str, status: str, *, actor: str) -> dict[str, Any]:
        ...
        with self.engine.begin() as connection:
            ...
            if allocation.status in {"liquidation_pending", "risk_reduction_pending"}:
                raise ValueError("pending allocation risk execution cannot be overridden manually")
            members = connection.execute(
                select(strategy_allocation_members).where(
                    strategy_allocation_members.c.allocation_id == allocation_id
                )
            ).all()
            portfolio_ids = [str(item.portfolio_id) for item in members if item.portfolio_id]
            if len(portfolio_ids) != len(members):
                raise ValueError("strategy allocation has an unprovisioned member")
            if status == "active":
                ...
                if unresolved is not None:
                    raise ValueError(
                        "critical allocation risk events must be resolved before reactivation"
                    )
                active_batch = connection.execute(
                    select(portfolio_batches.c.id)
                    .where(
                        portfolio_batches.c.portfolio_id.in_(portfolio_ids),
                ...
                if active_batch is not None:
                    raise ValueError(
                        "child portfolio batches must finish before allocation reactivation"
                    )
                child_rows = connection.execute(
                    select(paper_portfolios.c.status).where(
                        paper_portfolios.c.id.in_(portfolio_ids)
                    )
                ).scalars()
                if any(item not in {"active", "paused"} for item in child_rows):
                    raise ValueError(
                        "every child portfolio must be safely paused before reactivation"
                    )
                connection.execute(
                    update(paper_portfolios)
                    .where(paper_portfolios.c.id.in_(portfolio_ids))
                ...
                connection.execute(
                    ...
                    .where(
                        paper_portfolios.c.id.in_(portfolio_ids),
                        paper_portfolios.c.status == "active",
                    )
                ...
                for portfolio_id in portfolio_ids:
                    portfolio_status = connection.execute(
                        select(paper_portfolios.c.status).where(
                            paper_portfolios.c.id == portfolio_id
                        )
                    ).scalar_one()
                    synchronize_portfolio_schedules(
                        connection,
                        portfolio_id,
                        str(portfolio_status),
                        now=now,
                    )
            connection.execute(
                update(strategy_allocations)
        ...
        *,
        ...
        with self.engine.begin() as connection:
            ...
            if event.status == "resolved":
                raise ValueError("resolved allocation events cannot be acknowledged again")
            if event.status not in {"open", "acknowledged"}:
                raise ValueError("only open allocation risk events may be acknowledged")
        ...
        *,
        ...
        with self.engine.begin() as connection:
            ...
            if event is None:
                raise KeyError(event_id)
            if event.status == "resolved":
                return self._event_row(event)
            if event.status not in {"open", "acknowledged"}:
                raise ValueError("only open or acknowledged allocation events may be resolved")
            allocation = connection.execute(
                select(strategy_allocations)
                .where(strategy_allocations.c.id == allocation_id)
                .with_for_update()
            ...
            if len(portfolio_ids) != len(members):
                raise ValueError("strategy allocation has an unprovisioned member")
            child_rows = connection.execute(
                select(paper_portfolios.c.id, paper_portfolios.c.status).where(
                    paper_portfolios.c.id.in_(portfolio_ids)
                )
            ).all()
            if any(
                item.status in {"liquidation_pending", "risk_reduction_pending"}
                for item in child_rows
            ):
                raise ValueError(
                    "child risk execution must finish before allocation event resolution"
                )
            if event.severity == "critical" and allocation.status == "active":
                raise ValueError("pause the strategy allocation before resolving a critical event")
            active_batch = connection.execute(
                select(portfolio_batches.c.id)
                .where(
                    portfolio_batches.c.portfolio_id.in_(portfolio_ids),
            ...
            if active_batch is not None:
                raise ValueError(
                    "child portfolio batches must finish before allocation event resolution"
                )

            liquidation = event.rule == "max_drawdown_liquidate"
            reduction = event.rule in {"max_drawdown_reduce", "max_member_drawdown"}
            if liquidation:
                position = connection.execute(
                    select(paper_positions.c.instrument)
                    .where(paper_positions.c.portfolio_id.in_(portfolio_ids))
                    .limit(1)
                ).first()
                if position is not None:
                    raise ValueError(
                        "allocation liquidation cannot be resolved while positions remain"
                    )
            if reduction:
                for member in members:
                    has_position = connection.execute(
                        select(paper_positions.c.instrument)
                        .where(paper_positions.c.portfolio_id == member.portfolio_id)
                        .limit(1)
                    ).first()
                    ...
                    config = connection.execute(
                        select(strategy_versions.c.config_json).where(
                            strategy_versions.c.id == member.strategy_version_id
                        )
                    ).scalar_one()
                    exposure_limit = float(
                        dict(config or {}).get("drawdown_reduction_exposure", 0.50)
                    )
                    exposure = connection.execute(
                        select(portfolio_nav.c.exposure)
                        .where(portfolio_nav.c.portfolio_id == member.portfolio_id)
                        .order_by(portfolio_nav.c.trade_date.desc())
                        .limit(1)
                    ).scalar_one_or_none()
                    if exposure is None or float(exposure) > exposure_limit + 0.01:
                        raise ValueError("child exposure remains above its risk-reduction limit")
    ...
    def refresh(self, allocation_id: str) -> dict[str, Any]:
        ...
        with self.engine.begin() as connection:
            ...
            if allocation.status not in {
                ...
                raise ValueError("strategy allocation must be active or in risk execution")
            members = connection.execute(
                select(strategy_allocation_members).where(
                    strategy_allocation_members.c.allocation_id == allocation_id
                )
            ...
            for member in members:
                ...
                latest[str(member.portfolio_id)] = nav_row
            ...
            member_nav = {portfolio_id: float(item.nav) for portfolio_id, item in latest.items()}
            nav = _decimal(allocation.cash_reserve) + sum(
                (_decimal(value) for value in member_nav.values()),
                Decimal("0"),
            )
            ...
            annualized_volatility = (
                float(np.std(history, ddof=1) * np.sqrt(252.0)) if len(history) > 1 else 0.0
            )
            member_weights = {
                portfolio_id: float(_decimal(value) / nav) if nav else 0.0
                for portfolio_id, value in member_nav.items()
            }
            drawdown_loss = abs(min(drawdown, 0.0))
            next_status = allocation.status
            action: str | None = None
            if drawdown_loss > float(allocation.max_drawdown_liquidate) + 1e-12:
            ...
            elif drawdown_loss > float(allocation.max_drawdown_reduce) + 1e-12:
            ...
            for member in members:
                nav_row = latest[str(member.portfolio_id)]
                member_loss = abs(min(float(nav_row.drawdown), 0.0))
                member_action = action
                if member_loss > float(allocation.max_member_drawdown) + 1e-12:
                    member_action = member_action or "reduce"
                    next_status = (
                        next_status
                ...
                if member_action:
                    has_positions = connection.execute(
                        select(paper_positions.c.instrument)
                        .where(paper_positions.c.portfolio_id == member.portfolio_id)
                        .limit(1)
                    ).first()
                    child_status = (
                        "liquidation_pending"
                        if member_action == "liquidate" and has_positions
                        else "risk_reduction_pending"
                        if member_action == "reduce" and has_positions
                        else "paused"
                    )
                    connection.execute(
                        update(paper_portfolios)
                        .where(paper_portfolios.c.id == member.portfolio_id)
                        .values(status=child_status, updated_at=now)
                    )
                    synchronize_portfolio_schedules(
                        connection,
                        str(member.portfolio_id),
                        child_status,
                        now=now,
                    )
            connection.execute(
    ...
    def get(self, allocation_id: str) -> dict[str, Any]:
        with self.engine.connect() as connection:
            ...
            result["members"] = [row_dict(item) for item in member_rows]
            ...
            for item in nav_rows:
                ...
                row["member_nav"] = row.pop("member_nav_json")
                row["member_weights"] = row.pop("member_weights_json")
                result["nav_history"].append(row)
            ...
            result["events"] = []
            for item in events:
                result["events"].append(self._event_row(item))
        return result
    ...
    def list(self, limit: int = 100) -> list[dict[str, Any]]:
        with self.engine.connect() as connection:
            ...
            allocation_ids = [str(item) for item in ids]
        return [self.get(item) for item in allocation_ids]
    ...
    def refresh_for_portfolio(self, portfolio_id: str) -> list[dict[str, Any]]:
            ...
            allocation_ids = [
                ...
                for item in connection.execute(
                    select(strategy_allocation_members.c.allocation_id).where(
                        strategy_allocation_members.c.portfolio_id == portfolio_id
                    )
                ).scalars()
            ]
        ...
        for allocation_id in allocation_ids:
            try:
                results.append(self.refresh(allocation_id))
                ...
                with self.engine.begin() as connection:
                    self._event(
                        ...
                        event_type="allocation_refresh_failed",
                        severity="critical",
                        rule="aggregate_nav_refresh",
        ...
        *,
        ...
        connection.execute(
            insert(strategy_allocation_events).values(
                ...
                observed=observed,
                limit_value=limit,
                status="open" if severity == "critical" else "recorded",
